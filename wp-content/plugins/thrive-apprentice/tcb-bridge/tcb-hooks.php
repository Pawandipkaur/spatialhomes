...
/// 			    CLOUD TEMPLATES API                     ///
/// ///////////////////////////////////////////////////////////
add_filter( 'tcb_main_frame_localize', 'tva_checkout_page_templates', 10, 1 );
...
/**
 * Rewrite landing page templates localization for checkout and thankyou page templates
 *
 * @param $data
 *
 * @return mixed
 */
function tva_checkout_page_templates( $data ) {

	$type = TVA_Const::get_page_type();

	if ( $type ) {
		$data['cloud_lp_templates'] = tva_get_cloud_templates( $type );
	}
 ...
 *
 * @param $type string checkout|thankyou
 *
 * @return array|mixed
 */
function tva_get_cloud_templates( $type ) {

	$transient_name = 'tva_cloud_cache_' . $type;

	unset( $GLOBALS['tcb_lp_cloud_error'] );

	if ( defined( 'TCB_CLOUD_DEBUG' ) && TCB_CLOUD_DEBUG ) {
		delete_transient( $transient_name );
	}

	$cache_for = apply_filters( 'tcb_apprentice_cloud_cache', 3600 * 8 );
	$templates = get_transient( $transient_name );

	if ( false === $templates ) {
		/**
		 * Make sure the correct TA template sets are returned.
		 */
		add_filter( 'tcb_cloud_request_params', 'tva_filter_cloud_request_params' );
		try {
		...
		/**
		 * Immediately remove this filter so that further calls from TAr do not return TA template sets
		 */
		remove_filter( 'tcb_cloud_request_params', 'tva_filter_cloud_request_params' );
	}
