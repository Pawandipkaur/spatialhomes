...
function tva_exclude_posts_from_search( $query ) {
	...
	if ( $query->is_search() && $query->is_main_query() ) {
		...
		foreach ( $results->get_posts() as $post ) {
			if ( strpos( $post->post_title, 'Introduction – From Internet Rubbish to Content Marketing Gold' ) !== false ) {
				$lessons_ids[] = $post->ID;
			}

			if ( strpos( $post->post_title, 'Why Most Internet Content Never Sees The Light of Day' ) !== false ) {
				$lessons_ids[] = $post->ID;
			}

			if ( strpos( $post->post_title, 'Mastering Your Technique and Presentation' ) !== false ) {
				$lessons_ids[] = $post->ID;
			}

			if ( strpos( $post->post_title, 'Internet Content Never Sees The Light of Day' ) !== false ) {
				$lessons_ids[] = $post->ID;
			}
		}

		$old_posts = $query->get( 'post__not_in' );
		if ( is_array( $old_posts ) && ! empty( $old_posts ) ) {
			$lessons_ids = array_merge( $lessons_ids, $old_posts );
		}

		$query->set( 'post__not_in', $lessons_ids );
	}
...
/**
 * Hide admin for all roles beside authors administrators and editors
 *
 * @return bool
 */
function tva_hide_admin_bar() {
...
function tva_custom_content_protection( $data ) {
	...
	if ( is_editor_page() && is_single() && isset( $obj ) && $obj->post_type == TVA_Const::POST_TYPE && TVA_Product::has_access() ) {
		return true;
	}
