...
class TVA_Frontend_Controller extends TVA_REST_Controller {
	...
	public function mark_lesson( $request ) {
		...
		$course          = tva_get_term_data( get_term( $course_id ) );
		$completed       = $request->get_param( 'completed' );
		$learned_lessons = isset( $_COOKIE['tva_learned_lessons'] ) ? $_COOKIE['tva_learned_lessons'] : array();
		$post_arr        = wp_list_filter( $course->posts, array( 'ID' => $lesson_id ) );
		$post_arr        = array_values( $post_arr );
		$post            = $post_arr[0];
		$allowed         = tva_access_manager()->has_access_to_post( $post );
