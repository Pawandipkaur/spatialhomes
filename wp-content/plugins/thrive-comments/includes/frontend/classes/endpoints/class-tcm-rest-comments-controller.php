...
class TCM_REST_Comments_Controller extends WP_REST_Comments_Controller {
	...
	public function prepare_comment_for_database( $request ) {
		...
		$comment_fields = array(
			...
			'comment_approved'       => $request->get_param( 'comment_approved' ),
			'comment_author'         => $request->get_param( 'comment_author' ),
			'comment_author_email'   => $request->get_param( 'comment_author_email' ),
			'comment_author_url'     => $request->get_param( 'comment_author_url' ),
			'comment_content'        => $request->get_param( 'comment_content' ),
			'comment_date'           => $request->get_param( 'comment_date' ),
			'comment_karma'          => $request->get_param( 'comment_karma' ),
			'comment_parent'         => $request->get_param( 'comment_parent' ),
			'comment_type'           => $request->get_param( 'comment_type' ),
			'user_id'                => get_current_user_id(),
			//Meta fields for comments
			'level'                  => wp_filter_kses( $request->get_param( 'level' ) ),
			'comment_author_picture' => wp_filter_kses( $request->get_param( 'comment_author_picture' ) ),
	...
	public function rest_prepare_comment( $response, $comment, $request ) {
		...
		$avatar = get_comment_meta( $response->data['id'], 'comment_author_picture', true );
		if ( $avatar ) {
			$response->data['author_avatar_urls'] = $avatar;
			$response->data['social_avatar']      = 'true';
		} else {
			$response->data['author_avatar_urls'] = tcmh()->get_picture_url();
		}
