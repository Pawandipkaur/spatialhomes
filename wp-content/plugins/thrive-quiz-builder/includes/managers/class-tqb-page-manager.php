...
class TQB_Page_Manager {
	...
	function get_page_display_html( $points ) {
		...
		if ( ( $page->post_type == Thrive_Quiz_Builder::QUIZ_STRUCTURE_ITEM_RESULTS || $page->post_type == Thrive_Quiz_Builder::QUIZ_STRUCTURE_ITEM_OPTIN )
			...
			$variation['content'] = str_replace( Thrive_Quiz_Builder::QUIZ_RESULT_SHORTCODE, '', $variation['content'] );
		}
	...
	public function insert_result_shortcodes( $variation, $points ) {
		foreach ( $this->get_result_shortcodes( $points ) as $shortcode ) {
			if ( preg_match( '/' . $shortcode['initial'] . '/', $variation['content'] ) ) {
				$variation['content'] = str_replace( $shortcode['initial'], $shortcode['replacement'], $variation['content'] );
			}
		}
	 ...
	 * @return array
	 */
	public function get_result_shortcodes( $points ) {
		return array(
			0 => array(
				'initial'     => "[tqb_quiz_result result_type='whole_number']",
				'replacement' => "[tqb_quiz_result result_type='whole_number' points='" . json_encode( $points ) . "']",
			),
			1 => array(
				'initial'     => "[tqb_quiz_result result_type='one_decimal']",
				'replacement' => "[tqb_quiz_result result_type='one_decimal' points='" . json_encode( $points ) . "']",
			),
			2 => array(
				'initial'     => "[tqb_quiz_result result_type='two_decimal']",
				'replacement' => "[tqb_quiz_result result_type='two_decimal' points='" . json_encode( $points ) . "']",
			),
			3 => array(
				'initial'     => "[tqb_quiz_result result_type='default']",
				'replacement' => "[tqb_quiz_result result_type='default' points='" . json_encode( $points ) . "']",
			),
		);
	}
