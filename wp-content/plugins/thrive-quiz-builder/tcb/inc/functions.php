...
function tve_load_global_styles() {
	...
	$selector      = apply_filters( 'tcb_global_styles_selector', '#tve_editor' );
	$shared_styles = str_replace( $selector, '', $shared_styles );
...
function is_editor_page_raw( $check_ajax_request = false ) {
	 ...
	 */
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
		$is_editor_ajax = $check_ajax_request && ! empty( $_REQUEST['action'] ) && 'tcb_editor_ajax' === $_REQUEST['action'];
		...
		if ( tve_tcb__license_activated() || apply_filters( 'tcb_skip_license_check', false ) ) {
			...
			if ( apply_filters( 'tcb_user_can_edit', true, $post_id ) ) {
				...
				$tve_path_params = array(
					...
					'dynamic_links_categories'      => array_keys( $dynamicLinks ),
				);
...
function tve_get_shared_styles( $post_content = '', $for_media = '' ) {
	...
	$selector = apply_filters( 'tcb_global_styles_selector', '#tve_editor' );
	$output   = str_replace( '#tve_editor', $selector, $output );
